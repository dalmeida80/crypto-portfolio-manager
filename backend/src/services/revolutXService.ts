 ...
 * - GET /api/1.0/public/order-book/{symbol} - Get order book (requires auth)
 * 
...
export class RevolutXService {
   ...
   * Uses order book endpoint with symbol as path parameter
  ...
  async getTicker(symbol: string): Promise<{ bid: number; ask: number; mid: number }> {
    try {
      // Endpoint: GET /api/1.0/public/order-book/{symbol}
      // Symbol is a PATH parameter, not query parameter
      const symbolUpper = symbol.toUpperCase();
      const response = await this.makeAuthenticatedRequest(
        'GET',
        `/api/1.0/public/order-book/${symbolUpper}`
      );
